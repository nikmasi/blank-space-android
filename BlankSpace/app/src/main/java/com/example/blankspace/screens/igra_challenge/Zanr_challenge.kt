...
@Composable
fun Zanr_challenge(navController: NavController,selectedNivo:String,viewModelIgraSam:IgraSamViewModel){
    Box(modifier = Modifier.fillMaxSize().padding(top=52.dp)) {
        BgCard2()
        Spacer(Modifier.padding(top = 22.dp))
        Zanr_challenge_mainCard(navController,selectedNivo,viewModelIgraSam)
    }
}

@Composable
fun Zanr_challenge_mainCard(navController: NavController,selectedNivo: String,viewModelIgraSam:IgraSamViewModel) {
            ...
            SmallButton(
                onClick = {
                    if (selectedZanrovi.isNotEmpty()) {
                        ...
                        navController.navigate(Destinacije.Igra_challenge.ruta + "/$zanroviIds/$selectedNivoList/80/0")
                    } else {
