...
@Composable
fun IzborZanraUklanjanjePesme_mainCard(navController: NavController,viewModelUklanjanje: UklanjanjeViewModel) {
    val context = LocalContext.current
    val viewModel: ZanrViewModel = hiltViewModel()
    val uiState by viewModel.uiState.collectAsState()

    var selectedZanrovi by remember { mutableStateOf(-1) }
    var imeZanrovi by remember { mutableStateOf("") }
        ...
        Column(
            ...
            SmallButton(onClick = {
                if (selectedZanrovi!=-1) {
                    viewModelUklanjanje.dohvatiIzvodjaceZanrova(selectedZanrovi,imeZanrovi)
